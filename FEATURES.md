...
- a `CheckpointStatusProvider` seam for Backup-owned authoritative runtime checkpoint evidence without treating an operation ID as a credential;
- `FileCheckpointStatusStore`, a versioned durable Backup-owned checkpoint lifecycle/evidence store with an explicitly caller-supplied path, immutable accepted-submission correlation, bounded chronological status history, idempotent exact replays, strict request/operation/dataset/scope rebinding prevention, monotonic observation times, lifecycle-regression checks, non-regressing completed recovery evidence, explicit later failure recording, duplicate operation/request rejection, strict JSON parsing, private atomic-file publication, private Unix permission checks, and fail-closed handling of malformed, unsupported, non-regular, loosely permissioned, or uninitialized state;
...
This remains a source-level integration foundation. Authenticated transport, concrete GoreeCloud Identity decision verification, GoreeCloud Mesh delivery, runtime configuration and authorized administration of the durable stores, a real Backup checkpoint executor, safe runtime wiring that records accepted submissions without duplicate execution on persistence/retry failures, authoritative engine/repository adapters that write real recovery-point and integrity evidence, Sync pause/maintenance orchestration, restore staging/promotion, post-restore reconciliation, audit integration, user interfaces, target-environment testing, and production acceptance remain incomplete. See [`docs/goreecloud-sync-integration.md`](docs/goreecloud-sync-integration.md).
...
The source-level authority, authorization-ordering, durable dataset-scope mapping store, durable checkpoint lifecycle/evidence store, and restore-safety contract now exist, but no production service-to-service transport or runtime orchestration uses them yet. The implementation must still authenticate and authorize cross-product requests with concrete platform adapters, select and protect canonical durable-store locations, provide authorized store administration and audit, execute requested checkpoints through a real Backup engine adapter, durably record accepted execution without unsafe duplicate retries, collect authoritative checkpoint completion/integrity evidence from real Backup/repository systems, coordinate Sync maintenance state, stage and validate restores, promote recovered data deliberately, reconcile Sync state afterward, and prove failure behavior when either product is unavailable.
